...
export function Reports() {
  ...
  const filterAssetsForPeriod = () => {
    return assets.filter(a => {
      ...
      return matchWs && a.status === 'owned' && !a.parentAssetId;
    });
  };
    ...
    if (exportIncludeAssets) {
      ...
      if (periodAssets.length === 0) {
        ...
        periodAssets.forEach(a => {
          csvContent += `"${a.name}","${a.category}","${a.workspaceId || 'keluarga'}",${a.purchasePrice},${getAssetEffectiveValue(a, assets, transactions)},"${a.purchaseDate}","${(a.notes || '').replace(/"/g, '""')}"\n`;
        });
      }
      csvContent += `\n`;
    }
  ...
  const handleExportJSON = () => {
    ...
    const totalPhysicalAssets = periodAssets.reduce((sum, a) => sum + getAssetEffectiveValue(a, assets, transactions), 0);
    ...
    const backupData = {
      appName: "Noora Harmoni Finance (NOHARFIN)",
  ...
  const pdfTotalPhysicalAssets = periodAssets.reduce((sum, a) => sum + getAssetEffectiveValue(a, assets, transactions), 0);
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between items-start border-b-2 border-slate-900 pb-5 gap-3">
              <div>
                ...
                <p className="text-[11px] text-slate-500 mt-1">
                  {isId ? 'Generasi Otomatis Aplikasi NOHARFIN' : 'Automatically Generated by NOHARFIN'}
                </p>
              </div>
                    ...
                    <tbody>
                      {periodAssets.map((a, i) => (
                        <tr key={a.id || i} className="border-b border-slate-200">
                          ...
                          <td className="p-2 text-right font-semibold">{formatCurrency(getAssetEffectiveValue(a, assets, transactions))}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table></div>
